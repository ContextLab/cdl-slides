...
def copy_assets_alongside_output(output_html: Path, source_dir: Optional[Path] = None) -> None:
    ...
    Also copies animations/ directory if it exists (generated by manim blocks).
    ...
    Args:
        ...
        source_dir: Optional source directory for animations/ (defaults to output dir)
    """
    ...
    if source_dir is not None and source_dir.resolve() != output_dir.resolve():
        ...
        if src_animations.is_dir() and any(src_animations.iterdir()):
            ...
            if dst_animations.exists():
                shutil.rmtree(dst_animations)
            shutil.copytree(src_animations, dst_animations)
